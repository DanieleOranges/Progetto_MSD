...
n_param = floor(length(pks_acc) / every_other);    % number of parameters collected

%% Dimensionless Damping Calculation
log_decs = (1 / every_other) * log(pks_acc(1 : every_other : ((n_param - 1) ...
    * every_other)) ./ pks_acc((1 + every_other) : every_other : ...
    (n_param * every_other)));   % calculate logarithmic decrements

adim_damp = log_decs ./ (2 * pi);    % dimensionless damping array
adim_damp_time = pks_time(1 : every_other : ((n_param - 1) * every_other));
...
osc_period = (1 / every_other) * (pks_time((1 + every_other) : every_other : ...
    (n_param * every_other)) - pks_time(1 : every_other : ((n_param - 1 ) * ...
    every_other))); % oscillation period calculation

eigenfreq = 1 ./ osc_period;    % array of first eigenfrequencies
eigenfreq_time = linspace(0, max(time), length(osc_period));    % time array of first eigenfrequencies   
